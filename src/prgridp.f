      SUBROUTINE PRGRIDP(MOUNT,HEIGHT,PANL,U,V,ZETA,DIV,L3D,LN,GCTR)
C                                                                              
...
C
C     Input
C LOWER SURFACE HEIGHT FIELD
      REAL MOUNT(NLON+2,NLAT)
C HEIGHT FIELD
      REAL HEIGHT(NLON+2,NLAT)
...
C NORTHWARD WIND FIELD
      REAL V(NLON+2,NLAT)
C VORTICITY FIELD
      REAL ZETA(NLON+2,NLAT,L3D),OUTZETA(NLON+2,NLAT)
C DIVERGENCE FIELD
      REAL DIV(NLON+2,NLAT,L3D),OUTDIV(NLON+2,NLAT)
...
C
      ...
      DO J=1,NLAT
         ...
         DO I=1,NLON+2
            OUTZETA(I,J)=ZETA(I,J,LN)
            OUTDIV(I,J)=DIV(I,J,LN)
         ENDDO
      ENDDO
C
...
C
      ...
      WRITE(NCHGRID,'(8E15.6)') MOUNT
      WRITE(NCHGRID,'(8E15.6)') HEIGHT
      WRITE(NCHGRID,'(8E15.6)') OUTZETA
      WRITE(NCHGRID,'(8E15.6)') OUTDIV
